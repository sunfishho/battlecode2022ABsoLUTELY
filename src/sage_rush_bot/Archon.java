...
public class Archon extends RobotCommon{
    ...
    static boolean hasFinishedRelocation = false;
    ...
    */
    ...
    public void takeTurn() throws GameActionException {
        ...
        if (round > 2 && !hasFinishedRelocation){
            //keep on walking if we're in portable, and we're sufficiently far, or we're close but not too close and our square is bad
            if (rc.getMode() == RobotMode.PORTABLE && 
                (me.distanceSquaredTo(archonTarget) > 16 || (rc.senseRubble(me) > rc.senseRubble(archonTarget) + 10 && me.distanceSquaredTo(archonTarget) > 4))){
                tryToMove(archonTarget, 80);
                System.out.println("I TRIED TO MOVE");
            }
            else if (me.distanceSquaredTo(archonTarget) <= 16){
                if (rc.canTransform()){
                    rc.transform();
                    hasFinishedRelocation = true;
                }
            }
        }
        ...
        if(round == 2){
            int minArchonDistanceFromCenter = 10000;
            int rankOfStationaryArchon = -1;
            ...
            for (int archonIndex = 0; archonIndex < numArchonsAtStart; archonIndex++){
                ...
                if (minArchonDistanceFromCenter > archonLocationsInitial[archonIndex].distanceSquaredTo(new MapLocation(Util.WIDTH / 2, Util.HEIGHT / 2))){
                    rankOfStationaryArchon = archonIndex + 1;
                    minArchonDistanceFromCenter = archonLocationsInitial[archonIndex].distanceSquaredTo(new MapLocation(Util.WIDTH / 2, Util.HEIGHT / 2));
                }
            }
            System.out.println("REMAIN STATIONARY ARCHON #" + rankOfStationaryArchon);
            rc.writeSharedArray(32, rankOfStationaryArchon);
            if (rc.readSharedArray(32) != 0){
                if (rank != rc.readSharedArray(32)){
                    archonTarget = archonLocationsInitial[rc.readSharedArray(32) - 1];
                    hasFinishedRelocation = false;
                }
                else{
                    hasFinishedRelocation = true;
                }
            }
            //now transform and run towards it
        }

        if (rc.getMode() == RobotMode.TURRET && rc.canTransform() && !hasFinishedRelocation && round >= 2){
            rc.transform();
        }
        ...
        else if (numBuilders == 0 && round < 50 && rank == 1 && rc.canBuildRobot(RobotType.BUILDER, dir)) {
            rc.buildRobot(RobotType.BUILDER, dir);
            rc.writeSharedArray(20, targetArchon + 1);
            numBuilders++;
        } 
        else if (rc.canBuildRobot(RobotType.SOLDIER, dir) && rc.readSharedArray(31) == 2) {
    ...
    public boolean tryToMove(MapLocation target, int avgRubble) throws GameActionException {
        Direction dir = Direction.CENTER;
        if (target != null){
