...
public class Archon extends RobotCommon{
    ...
    static int farthestArchonFromCenterRank;
    ...
    */
    ...
    public void initializeEachTurn() throws GameActionException{
        ...
        farthestArchonFromCenterRank = -1;
        int bestDistance = -1;
        for (int idx = 0; idx < numArchons; idx++){
            if (bestDistance < Util.distanceMetric(new MapLocation(Util.WIDTH / 2, Util.HEIGHT / 2), Util.getLocationFromInt(rc.readSharedArray(idx)))){
                ...
                farthestArchonFromCenterRank = idx + 1;
            }
        }
        ...
        if (aggregateHealthQueue.size() == 10){
            ...
            if (rc.readSharedArray(40) >= aggregateHealthQueue.peek() && aggregateHealthQueue.peek() > 200 && rank == farthestArchonFromCenterRank && round >= 60){
    ...
    public void tryToBuildStuff(Direction dir, boolean danger, int prevIncome) throws GameActionException { 
        ...
        if (!built && numBuilders == 0 && (labValue % 10000) % 101 != 0 && rc.canBuildRobot(RobotType.BUILDER, dir) && rank == farthestArchonFromCenterRank) {
        ...
        if((!danger || shouldFarm) && rank == farthestArchonFromCenterRank && numDefenders > 0) { //incorporate foundMiner at some point
    ...
    public void moveIfArchonHasTarget() throws GameActionException{
        ...
        boolean isTargetArchonLocation = false;
        for (int idx = 0; idx < numArchons; idx++){
            if (Util.getLocationFromInt(rc.readSharedArray(idx)).equals(archonTarget)){
                isTargetArchonLocation = true;
                break;
            }
        }
        if (isTargetArchonLocation){
            if (me.distanceSquaredTo(archonTarget) <= 25){
                archonTarget = me;
    ...
    public void checkIfArchonShouldRelocate() throws GameActionException{
        ...
        int closestArchonFromCenterRank = -1;
        int minDistance = 1000000;
        for (int idx = 0; idx < numArchons; idx++){
            if (minDistance >= Util.distanceMetric(new MapLocation(Util.WIDTH / 2, Util.HEIGHT / 2), Util.getLocationFromInt(rc.readSharedArray(idx)))){
                minDistance = Util.distanceMetric(new MapLocation(Util.WIDTH / 2, Util.HEIGHT / 2), Util.getLocationFromInt(rc.readSharedArray(idx)));
                closestArchonFromCenterRank = idx + 1;
            }
        }
        MapLocation closestArchonLoc = Util.getLocationFromInt(rc.readSharedArray(closestArchonFromCenterRank - 1));
        MapLocation farthestArchonLoc = Util.getLocationFromInt(rc.readSharedArray(farthestArchonFromCenterRank - 1));
        if (rank == closestArchonFromCenterRank && closestArchonLoc.distanceSquaredTo(farthestArchonLoc) >= 25 && Util.distanceMetric(closestArchonLoc, farthestArchonLoc) < Util.max(Util.HEIGHT, Util.WIDTH) * 0.6){
            //we should relocate the archon
            archonTarget = farthestArchonLoc;
    ...
    public boolean observe() throws GameActionException {
        for (RobotInfo robot : rc.senseNearbyRobots(34, enemyTeam)) {
            switch (robot.getType()){
                case MINER: continue;
                case ARCHON: 
                    rc.writeSharedArray(53, Util.getIntFromLocation(robot.location) + 10000 * rankOfNearestArchon(robot.getLocation()));
